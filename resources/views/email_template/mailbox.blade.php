@extends('admin.layout.app',['pageTitle' => __('Template Management')]) @section('content') @component('admin.layout.inc.breadcrumb') @slot('title') {{ __('Email Templates') }} @endslot @endcomponent @include('elements.alert')
<div class="row">
                                ...
                                <tr class="{{$mail->status}}">
                                    ...
                                    <td class="max-texts">
                                        <a href="{{ url('settings/mailbox/detail/'.$mail->id) }}"><span class="label label-info m-r-10">{{$mail->template}}</span>
                                            {{Core::limit_text(strip_tags($mail->message),19)}}
                                        </a>
                                    </td>
